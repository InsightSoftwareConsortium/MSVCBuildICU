...
function(midas_api_login)
    ...
    # Sanity check set(expected_nonempty_vars API_URL API_EMAIL API_KEY RESULT_VARNAME)
...
function(midas_api_item_upload)
    ...
    midas_api_item_create( API_URL ${MY_API_URL} API_EMAIL ${MY_API_EMAIL} API_KEY ${MY_API_KEY} PARENTID ${MY_FOLDERID} NAME ${filename} RESULT_VARNAME itemid )
    file(MD5 ${MY_ITEM_FILEPATH} checksum)
    midas_api_upload_generatetoken( API_URL ${MY_API_URL} API_EMAIL ${MY_API_EMAIL} API_KEY ${MY_API_KEY} ITEMID ${itemid} FILENAME ${filename} CHECKSUM ${checksum} RESULT_VARNAME generatetoken )
    midas_api_upload_perform( API_URL ${MY_API_URL} UPLOADTOKEN ${generatetoken} FOLDERID ${MY_FOLDERID} ITEMID ${itemid} ITEM_FILEPATH ${MY_ITEM_FILEPATH} RESULT_VARNAME output )
...
function(midas_api_upload_perform)
 ...
 # and the token returned is blank, the server already has this file and there # is no need to upload.
 ...
 function(midas_api_upload_generatetoken)
    ...
    midas_api_login( API_URL ${MY_API_URL} API_EMAIL ${MY_API_EMAIL} API_KEY ${MY_API_KEY} RESULT_VARNAME midas_api_token )
    midas_api_escape_for_url(itemid "${MY_ITEMID}")
...
function(midas_api_file_size filepath varname)
    ...
    set(_offset 0) _read_chunk(${filepath} ${_offset} _current_chunk_size)
    set(_offset ${_current_chunk_size})
    while(_current_chunk_size EQUAL ${_expected_chunk_size}) _read_chunk(${filepath} ${_offset} _current_chunk_size)
...
function(midas_api_package_upload)
    include(CMakeParseArguments)
    set(options)
    set(oneValueArgs API_URL API_EMAIL API_KEY FILE FOLDER_ID APPLICATION_ID OS ARCH REVISION SUBMISSION_TYPE CODEBASE PRODUCT_NAME PACKAGE_TYPE RELEASE NAME RESULT_VARNAME )
    set(multiValueArgs)
    cmake_parse_arguments(MY "${options}" "${oneValueArgs}" "${multiValueArgs}" ${ARGN})
    # Sanity check
